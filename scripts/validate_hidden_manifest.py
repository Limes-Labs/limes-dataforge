...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a DataForge trusted hidden-shard manifest.")
    ...
    report = {
        ...
        "shard_count": len(manifest.get("shards", [])) if isinstance(manifest.get("shards"), list) else None,
        "errors": errors,
    }
