...
def current_manifest_ledger_and_bundle() -> tuple[dict, dict, dict]:
    ...
    changed_paths = ["solution/filter.py"]
    ...
    selected["public_proxy_loss"] = score["public_proxy_loss"]
    selected["baseline_public_proxy_loss"] = score["baseline_public_proxy_loss"]
    selected["stress_public_proxy_loss"] = score["stress_public_proxy_loss"]
    selected["selection_hash"] = score["selection_hash"]
    selected["dataset_hash"] = score["dataset_hash"]
    selected["invariant_probes_passed"] = probes["ok"]
    ledger["candidate_selection"]["selected_score"] = score["public_proxy_loss"]
    ledger["candidate_selection"]["baseline_score"] = score["baseline_public_proxy_loss"]
...
class LocalBundleGuardTests(unittest.TestCase):
    ...
    def test_accepts_current_score_probe_and_ledger_bundle(self) -> None:
    ...
    def test_rejects_stale_public_score_hash(self) -> None:
        manifest, ledger, source_bundle = current_manifest_ledger_and_bundle()
        manifest["public_score"]["selection_hash"] = "0" * 64
        report = validate_local_bundle(
            manifest=manifest,
            contract=self.contract,
            changed_paths=["solution/filter.py"],
        ...
        self.assertTrue(any("public_score.selection_hash" in error for error in report["errors"]))
