...
class SearchLedgerGuardTests(unittest.TestCase):
    ...
    def test_rejects_hidden_data_selection(self) -> None:
        ledger = copy.deepcopy(self.ledger)
        ledger["candidate_selection"]["selected_after_seeing_hidden_data"] = True
        errors = validate_ledger(ledger)
        self.assertTrue(any("selected_after_seeing_hidden_data" in error for error in errors))

    def test_actual_attempts_must_match_attempt_list(self) -> None:
        ledger = copy.deepcopy(self.ledger)
        ledger["search_budget"]["actual_attempts"] = 3
        errors = validate_ledger(ledger)
        self.assertTrue(any("actual_attempts" in error for error in errors))
    ...
    def test_cli_rejects_bad_ledger(self) -> None:
        ledger = copy.deepcopy(self.ledger)
        ledger["public_data_boundary"]["used_hidden_data"] = True
        ...
        self.assertIn("used_hidden_data", result.stdout)
