...
REQUIRED_SELECTION = {
    ...
    "selection_reason",
    "stopped_reason",
    "selected_after_seeing_hidden_data",
}
REQUIRED_ATTEMPT = {
    "id",
    "status",
...
REQUIRED_BOUNDARY = {
    "used_hidden_data",
    "used_private_leaderboard_feedback",
    "public_smoke_candidate_only",
}
REQUIRED_REPLAY = {"requested", "rationale"}
...
def _validate_budget(budget: Any) -> list[str]:
    ...
    for field in ["network_used", "external_data_used"]:
...
def _validate_selection(selection: Any) -> list[str]:
    ...
    if selection.get("selection_metric") != "public_proxy_loss":
        errors.append("candidate_selection.selection_metric must be public_proxy_loss")
    ...
    if selection.get("selected_after_seeing_hidden_data") is not False:
        errors.append("candidate_selection.selected_after_seeing_hidden_data must be false")
    return errors
...
def _validate_attempt(attempt: Any, index: int) -> list[str]:
    ...
    for field in ["hypothesis", "command", "selection_hash", "dataset_hash", "result_summary"]:
    ...
    for field in ["public_proxy_loss", "baseline_public_proxy_loss", "stress_public_proxy_loss"]:
        if not _is_number(attempt.get(field)):
            errors.append(f"{prefix}.{field} must be numeric")
    if not isinstance(attempt.get("invariant_probes_passed"), bool):
        errors.append(f"{prefix}.invariant_probes_passed must be boolean")
...
def validate_ledger(ledger: dict[str, Any]) -> list[str]:
    ...
    if missing:
        ...
        return errors
    if ledger.get("challenge") != "limes-dataforge":
        errors.append("search ledger challenge must be limes-dataforge")
    ...
    if isinstance(selection, dict):
        ...
        if selected_attempt is not None:
            if not _almost_equal(selection.get("selected_score"), selected_attempt.get("public_proxy_loss")):
                errors.append("candidate_selection.selected_score must match selected attempt public_proxy_loss")
            if not _almost_equal(selection.get("baseline_score"), selected_attempt.get("baseline_public_proxy_loss")):
                errors.append("candidate_selection.baseline_score must match selected attempt baseline_public_proxy_loss")
    ...
    if not isinstance(boundary, dict):
        ...
        if boundary.get("used_hidden_data") is not False:
            errors.append("public_data_boundary.used_hidden_data must be false")
        if boundary.get("used_private_leaderboard_feedback") is not False:
            errors.append("public_data_boundary.used_private_leaderboard_feedback must be false")
        if boundary.get("public_smoke_candidate_only") is not True:
            errors.append("public_data_boundary.public_smoke_candidate_only must be true")
