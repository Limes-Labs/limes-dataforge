...
def validate_packet(packet: dict[str, Any], contract: dict[str, Any]) -> list[str]:
    ...
    if "seed_count" in evidence and (not isinstance(evidence["seed_count"], int) or evidence["seed_count"] <= 0):
        errors.append("evidence.seed_count must be a positive integer")
    ...
    if requested_status in PROMOTED_OR_LATER and evidence.get("downstream_non_regression") is not True:
        errors.append("evidence.downstream_non_regression must be true for promoted or later status")
