...
def current_public_baseline(config_path: Path = DEFAULT_CONFIG) -> dict[str, Any]:
    score = public_score(config_path)
    probes = run_invariant_probes(config_path)
    audit = run_public_audit(config_path)
    return {
        "schema_version": SCHEMA_VERSION,
        "challenge": "limes-dataforge",
        "baseline_kind": BASELINE_KIND,
        "description": "Locks stable public smoke fields for benchmark drift checks; not an official leaderboard baseline.",
        ...
        "dataset_hash": score.get("dataset_hash"),
        "stress_dataset_hash": score.get("stress_dataset_hash"),
    }
...
def validate_public_baseline(
    ...
    if current is None:
        current = current_public_baseline(config_path)
    errors: list[str] = []
    ...
    for field in ["skipped_score_fields", "invariant_probes", "public_audit", "dataset_hash", "stress_dataset_hash"]:
