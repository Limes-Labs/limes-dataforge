...
def validate_result(payload: dict[str, Any], contract: dict[str, Any]) -> list[str]:
    ...
    for field in ["hidden_val_loss", "public_val_loss", "baseline_hidden_val_loss"]:
        if field in metrics and not _is_number(metrics[field]):
            errors.append(f"metrics.{field} must be numeric")
    if "hidden_val_loss" in metrics and _is_number(score) and metrics["hidden_val_loss"] != score:
        errors.append("score must equal metrics.hidden_val_loss")
    if "seed_count" in metrics and (not isinstance(metrics["seed_count"], int) or metrics["seed_count"] <= 0):
        errors.append("metrics.seed_count must be a positive integer")
    if metrics.get("seed_count") != replay.get("seed_count"):
        errors.append("metrics.seed_count must match replay.seed_count")

    missing_replay = _missing_fields(replay, REQUIRED_REPLAY_FIELDS)
    if missing_replay:
        errors.append("replay missing fields: " + ", ".join(missing_replay))
    if "seed_count" in replay and (not isinstance(replay["seed_count"], int) or replay["seed_count"] <= 0):
        errors.append("replay.seed_count must be a positive integer")
    for field in ["trusted_runner", "run_id", "code_hash", "dataset_hash", "verifier_contract_hash", "dependency_lock"]:
    ...
    if metrics.get("dataset_hash") != replay.get("dataset_hash"):
        errors.append("metrics.dataset_hash must match replay.dataset_hash")
    ...
    if status == "scaled" and promotion.get("scaled_audit") is not True:
        errors.append("promotion.scaled_audit must be true for scaled status")
    return errors
