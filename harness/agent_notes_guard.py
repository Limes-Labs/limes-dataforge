...
def _validate_public_score(score: Any, prefix: str) -> list[str]:
    ...
    if missing:
        return [f"{prefix}.public_score missing fields: {', '.join(missing)}"]
    return []
...
def _validate_attempt(attempt: Any, index: int) -> list[str]:
    ...
    for field in ("hypothesis", "command", "result_summary"):
...
def validate_notes(payload: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if payload.get("challenge") != "limes-dataforge":
        errors.append("agent notes challenge must be limes-dataforge")
    ...
    if not _non_empty_string_list(selection_summary.get("known_overfit_risks")):
        errors.append("selection_summary.known_overfit_risks must be a non-empty list")
