...
def _validate_real_artifacts(packet: dict[str, Any], contract: dict[str, Any], root: Path) -> list[str]:
    ...
    if isinstance(metrics, dict):
        comparisons = {
            "public_proxy_loss": public_score.get("public_proxy_loss"),
            "baseline_public_proxy_loss": public_score.get("baseline_public_proxy_loss"),
            "public_proxy_improvement": public_score.get("public_proxy_improvement"),
            "stress_public_proxy_loss": public_score.get("stress_public_proxy_loss"),
        }
        ...
        for field in ["selection_hash", "dataset_hash"]:
    ...
    artifact_validations = {
        ...
        "public_score": isinstance(public_score, dict),
...
def validate_packet(
    ...
    if not isinstance(evidence, dict):
        ...
        if evidence.get("public_smoke_candidate_only") is not True:
            errors.append("evidence.public_smoke_candidate_only must be true")
...
def build_candidate_packet(
    ...
    artifacts = {
        ...
        "public_score": artifact_record(public_score_path, root, isinstance(public_score, dict)),
    ...
    evidence = {
        ...
        "public_smoke_candidate_only": True,
    }
    failed = [
        name
    ...
    packet = {
        ...
        "local_metrics": {
            ...
            "public_proxy_improvement": public_score.get("public_proxy_improvement"),
            "stress_public_proxy_loss": public_score.get("stress_public_proxy_loss"),
            "selection_hash": public_score.get("selection_hash"),
            "dataset_hash": public_score.get("dataset_hash"),
            "source_bundle_sha256": source_bundle.get("bundle_sha256"),
        },
        "gates": {
            "passed": [name for name, passed in evidence.items() if passed is True],
            "failed": failed,
            "blocked": ["trusted replay not run", "hidden verifier not run"],
        },
    }
    return packet
