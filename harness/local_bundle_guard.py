...
SKIPPED_PUBLIC_SCORE_FIELDS = ["runtime_seconds"]
...
def _compare_search_ledger(ledger: Any, manifest_score: Any) -> list[str]:
    ...
    selection = ledger["candidate_selection"]
    if not _same_value(selection.get("selected_score"), manifest_score.get("public_proxy_loss")):
        errors.append("search_ledger selected_score must match manifest public_proxy_loss")
    if not _same_value(selection.get("baseline_score"), manifest_score.get("baseline_public_proxy_loss")):
        errors.append("search_ledger baseline_score must match manifest baseline_public_proxy_loss")
...
def validate_local_bundle(
    ...
    fresh_score = public_score(config_path)
